...
SDE.Ghost = function() {
    var LANGUAGE_CODE = (typeof EC_GLOBAL_INFO === 'object' && typeof EC_GLOBAL_INFO.getAdminLanguageCode === 'function')
        ? EC_GLOBAL_INFO.getAdminLanguageCode()
        : 'ko_KR';
    var BTN_WIDTH = 170,
        BTN_HEIGHT = 67,

        DUMMY_SELECTOR = '.xans-layout-dummy',
        DUMMY_TEMPLATE = '<div style=\'padding:65px 0 75px; border:1px solid #ffdeb7; background:url("//img.echosting.cafe24.com/design/common/bg_add_module_line.gif") repeat 0 0; text-align:center;\'>' +
                            '<p style=\'background:url("//img.echosting.cafe24.com/design/common/bg_add_module.png") no-repeat 50% 0; padding:170px 0 0;\'>' +
                                '<img src="//img.echosting.cafe24.com/design/common/'+ LANGUAGE_CODE +'/txt_add_module.gif" alt="화면을 편집하려면 여기를 클릭하세요. 모듈을 추가하고 꾸밀 수 있습니다.">' +
                            '</p>' +
                         '</div>',

        TEMPLATE       = '<div class="field" style="display:none;">' +
                             '<div class="editorBtn" style="padding: 0 35px 0 0;">' +
                                 '<p class="btn"><button type="button" class="edit">편집</button></p>' +
                                 '<p class="info">SIZE : <span></span></p>' +
                                 '<div class="arrowBtn" style="position: absolute;right: 20px;top: 8px;"><a href="#" class="up" data-cmd="up">up</a><a href="#" class="down" data-cmd="down">down</a></div>' +
                             '</div>' +
                             '<p class="closeBtn"><button type="button" class="close" title="닫기">닫기</button></p>' +
                         '</div>';

    var $ghost, $module,
        isLocked = false,
        layerModule, layerEditing, utilModule, fileManager;


    var action = function(actionType) {
        var fn = actionType ? eval('action' + actionType.charAt(0).toUpperCase() + actionType.slice(1)) : null,
            key = getModuleKey(),
            type = getModuleType(),
            index = getModuleIndex(key, type),
            result;

        if (!utilModule.has(type, key)) {

            result = utilModule.find(type, key, index);

            if (!result || !result.file) {
                alert("해당 모듈을 찾을 수 없습니다.  HTML 태그가 올바르게 닫혀있는지 확인해주세요");
            } else {
                fileManager.open(result.file, { key : key, type : type, index : result.index, action : actionType });
            }
            return;
        }

        utilModule.select(type, key, index);
     ...
     * 선택된 모듈 디자인 수정
     */
    var actionEdit = function(type, key) {
        layerEditing.show(type, key);
    };

    /**
     * 선택된 모듈 삭제
     */
    var actionClose = function(key) {
        if (!confirm("선택한 모듈을 삭제 하시겠습니까?")) return;

        utilModule.deleteSelection();
    ...
    var getModuleType = function() {
        if ($module && $module.attr('tagName') == 'IMG') {
            return 'image';
        }
     ...
     * Module Key 반환
     *
     * 'image'인 경우 src, 모듈인 경우 module 이름
     */
    var getModuleKey = function() {
        var key = '';

        try {
            if ($module.attr('tagName') == 'IMG') {
                key = $module.attr('src') || $module.attr('data-src'); // lazy load src 지원
            } else {
                key = $module.attr('class').split(' ')[2].slice(5).replace(/-/g, '_').toLowerCase();
            }
        } catch (e) {}
    ...
    /**
     * 모듈 Jquery Element 반환
     */
    var getModule = function(key, type) {
        var selector;

        if (type == 'module') {
            selector = '.xans-' + key.toLowerCase().replace(/_/g, '-');
        } else {
            selector = 'img[src="'+ key +'"]';
        }
    ...
    /**
     * 현재 선택된 Module의 Index 반환. (중복 모듈이 있는 경우를 대비)
     */
    var getModuleIndex = function(key, type) {
        var $target = getModule(key, type), index;

        if ($target.length == 0) return;

        // 스와이퍼 플러그인에서 duplicate 엘리먼트를 만들어서 동작하여 제거처리함
        if (type == 'image' && $target.length > 0 && $($target[0]).closest('.xans-editor-dndswiper').length > 0) {
            return 0;
        }
        for (index in $target) {
            if ($target[index] == $module[0])
                return index;
        }
    ...
    /**
     * 현재 선택된 모듈의 위치를 파악하여, Button을 보여줄 장소를 계산
     */
    var getPosition = function(offset, width, height) {
        var className = ['field'],
            $window = $(window);

        if (offset.left + BTN_WIDTH >= $window.width()) {
            className.push('right');
        } else if (offset.top + height + BTN_HEIGHT >= $window.scrollTop() + $window.height()) {
     ...
     * Ghost Layer hide
     */
    var hide = function($bImmediate) {
        if ($bImmediate) {
            $ghost.hide();
            $module = null;
        } else {
            $ghost.delay(500).fadeOut(100, function() {
                $module = null;
            });
        }
    };
    ...
    var init = function() {
        ...
        if (!parent.SDE) return;

        // [솔업2차] - 2013.11.07
        // ECHOSTING-53270 아래의 스크립트가 앞에서 로드되지 않았다면 스마트디자인 편집창이 아니라고 판단 하여 현재의 스크립트를 중지
        try {
            if (typeof parent.SDE.Util.Module == 'undefined' ||
                typeof parent.SDE.File.Manager == 'undefined' ||
                typeof parent.SDE.Layer.Editing == 'undefined') return;
        } catch (e) {
            return;
        }
        ...
        utilModule = parent.SDE.Util.Module;
        fileManager = parent.SDE.File.Manager;
        layerEditing = new parent.SDE.Layer.Editing();

        setDummy();

        if (params.key) {
            select(params.key, params.type, params.index, params.action);
        }

        setEventHandler();
        parent.SDE.Prop.lang = SHOP.getLanguage();
        $(document.body).css({"overflow-y":"scroll"});
    };


    /**
     * Ghost Layer HTML Render & Set Event
     */
    var render = function() {
        $ghost = $(TEMPLATE).appendTo(document.body);

        $size = $ghost.find('.info span');

        $ghost.find('button').click($.proxy(onClickButton, this));

        $ghost
            .click($.proxy(onClickGhost, this))
            .mouseleave($.proxy(onLeaveGhost, this));

        if (SDE.mo()) {
            $ghost
                .find('.editorBtn').css({padding: '0 35px 0 0'}).end()
                .find('.arrowBtn').css({position: 'absolute', right: 20, top: 8})
                    .find('a').css({display: "block", overflow: "hidden", width: 28, height: 18, background: 'url("//img.echosting.cafe24.com/smartAdmin/img/editor/ko_KR/sfix_btn.png") no-repeat', "text-indent": '150%', cursor: 'pointer'})
                    .click($.proxy(onClickArrange, this))
                        .filter('.up').css({'background-position': '-366px -481px'}).end()
                        .filter('.down').css({'background-position': '-366px -509px'})
            ;
        } else {
            $ghost
                .find('.arrowBtn').hide()
    ...
    /**
     * Ghost Layer 이용을 위해 링크와 그 외 동작들을 제어
     */
    var restrict = function() {
        ...
        // Floating Banner 움직이지 않게 처리
        setTimeout(function() { $(window).unbind('scroll'); }, 300);
    };
    ...
    var show = function($target) {
        var offset = $target.offset(),
            width = $target.outerWidth() + (SDE.mo()? -8: 0),
            height = $target.outerHeight() + (SDE.mo()? -8: 0),
            position = getPosition(offset, width, height);

        $module = $target;

        if (!$target.is(':visible') || $target.is(':animated')) return;

        if (offset.left == 0 && offset.top == 0 && $(window).height() < $module.height()) {
            $ghost
                .attr('class', 'field')
                .css({
                    top: offset.top + 4,
                ...
                /*rev.b16.20130828.6@sinseki #SDE-7: 모듈 편집 클릭시 간헐적인 "해당 모듈을 찾을 수 없습니다.  HTML 태그가 올바르게 닫혀있는지 확인해주세요" 경고창 오류*/
            ...
            $ghost
                .attr('class', position)
                .css({
                    top: offset.top + (SDE.mo()? 4: 0),
                    left: offset.left + (SDE.mo()? 4: 0),
                    width: width,
                    height: height
                /*rev.b16.20130828.6@sinseki #SDE-7: 모듈 편집 클릭시 간헐적인 "해당 모듈을 찾을 수 없습니다.  HTML 태그가 올바르게 닫혀있는지 확인해주세요" 경고창 오류*/
                }).stop(true, true).fadeIn(200, function (){
        ...
        if (SDE.mo()) {
            $ghost.find(".closeBtn").css({right:-4,top:-14});
        }

        $size.html(width + 'px * ' + height + 'px');

        //inner($target);

        if (SDE.mo() && $target.closest("#contents")[0] && $target.attr('tagName') != 'IMG') {
            var $surround = check($target);
            $ghost
                .find('.editorBtn').css({padding: '0 35px 0 0'})
            .end()
                .find('.arrowBtn').show()
                    .find('a')
                        .filter('.up').attr("data-cmd",$surround.prev? "up" :"").css({opacity:$surround.prev?1:.3})
                    .end()
                        .filter('.down').attr("data-cmd",$surround.next? "down" :"").css({opacity:$surround.next?1:.3});
        } else {
            $ghost.find('.editorBtn').css({padding: '0'}).end().find('.arrowBtn').hide();
        }

    };

    var inner = function($target) {
        var $containerOffset = $target.offset();
        $ghost.find(".innermodule").remove();
        $target.find('.xans-element-').filter(":not(.xans-record-)").each(function($i, $item){
            var t;
            var $jitem = $($item);
            if ($jitem.css("position") == "fixed") return;
            $('<div><div class="-panel"></div><div class="-cmd" style="cursor:pointer !important;_cursor:hand;position:absolute;right:-2px;top:-2px;width:8px;height:8px;border:2px solid #f44;"></div></div>').addClass("innermodule")
            .css({
                position: "absolute",
                left: (t = ($jitem.offset().left - $containerOffset.left), t-(SDE.mo()? 4: 0)),
                top: (t = ($jitem.offset().top - $containerOffset.top), t-(SDE.mo()? 4: 0)),
                width: $jitem.outerWidth() - 4,
                height: $jitem.outerHeight() - 4,
    ...
    /**
     * Ghost Layer 선택 후 편집 Layer Open
     *
     * module 관련 파라미터가 넘어 왔을 때 사용
     */
    var select = function(key, type, index, actionType) {
        var selector, offset, $target,
            index = index || 0;

        type = type || 'module';
        $target = getModule(key, type).eq(index);
    ...
    /**
     * Layout_Dummy 모듈 TEMPLATE 설정
     *
     * Editor 창에서만 보이도록 Ghost에서 html 삽입
     */
    var setDummy = function() {
        var $dummy = $(DUMMY_SELECTOR);

        if ($dummy.length == 0) return;

        $dummy.html(DUMMY_TEMPLATE);
    };

    var setEventHandler = function() {
        var oList = $('.xans-element-, img').filter(function (sTarget) {
            return this.className.split(' ').filter(function(sClassName) {
                return sClassName == 'xans-editor-dndimage' || sClassName == 'xans-editor-dndswiper';
            }).length < 1;
        });

        oList.mouseover($.proxy(onOverModule, this));
    };

    var onClickButton = function(evt) {
        var actionType = $(evt.currentTarget).attr('className');

        action(actionType);
    };

    var onOverModule = function(evt) {
        evt.stopPropagation();
        var $target = $(evt.currentTarget);
        /*rev.b2.20130819.11@sinseki #SDE-12 모듈 아닌 노드를 모듈이나 이미지로 인식해 편집레이어가 뜨는 오류*/
        //if (isLocked || $target.css('position') == "absolute" || $target.is(':animated') || $target.parents('.xans-element-').length != 0) return;
        if (isLocked || $target.css('position') == "absolute" || $target.is(':animated')) {
            return;
        }

        var $parents = $target.parents('.xans-element-').filter(function () {
            return this.className.split(' ').filter(function (sClassName) {
                return sClassName == 'xans-editor-dndimage' || sClassName == 'xans-editor-dndswiper';
            }).length < 1;
        });
        if ($parents.length) {
            show($parents.eq($parents.length-1));
        } else {
            show($target);
        }
    ...
    var onClickGhost = function(evt) {
        action();
    };

    var onLeaveGhost = function(evt) {
        if ($.contains($ghost[0], evt.toElement)) return;

        hide();
    };

    var onClickArrange = function(evt) {
        evt.stopPropagation();
        var $dir = {up:-1,down:1}[$(evt.target).data("cmd")];
        $dir && write($module,$dir);
    };
    ...
    var splitter = function ($target) {
        var SDEE = parent.SDE.editor;
        var key, type = "module";
        try {
            key = ($target.attr('tagName') == 'IMG' && (type = 'image'))?
                $target.attr('src'):
                $target.attr('class').split(' ')[2].slice(5).replace(/-/g, '_').toLowerCase();
        } catch (e) {
        }
        if (!key || type == 'image') return;
        var index = getModuleIndex(key, type);
        var range = SDEE.getModuleRange(key, type);
        if (!range) return;
        var $text = SDEE.getValue().split(/\n/);
        $text[range.from.line] = $text[range.from.line].replace(/(data-selected=""\s+)?\b(module=)/,'data-selected="" $2');
        var $atok = parent.exports.htmlutil.tokenize($text.join("\n"));
        var $parent = null;
        var $head = null;
        $atok.forEach(function($item,$k){
            if ($parent) return;
            if (/data-selected="" module=/.test($item[2])) {
                $parent = "/"+$atok[$k][1].join("/");
            }
        });
        var $splitter = {before:[],target:[],after:[]};
        var $qqq = [];
        $atok.forEach(function($item,$k){
            if (!$head && !/^TX/.test($atok[$k][0]) && $parent == "/"+$atok[$k][1].join("/")) {
                $head = "/"+$atok[$k][1].join("/") + ($atok[$k][1].length?"/":"")+$atok[$k][3];
                $qqq.push($item[2]);
            } else if ($head) {
                $qqq.push($item[2]);
                if ($head == "/"+$atok[$k][1].join("/") + ($atok[$k][1].length?"/":"")+$atok[$k][3]) {
                    if ($splitter.after.length) {
                        $splitter.target.push($splitter.target.pop()+$splitter.after.join(""));
                        $splitter.after = [];
                    }
                    $splitter.target.push($qqq.join(""));
                    $qqq = [];
                    $head = null;
                }
            } else {
                if ($splitter.target.length) {
                    $splitter.after.push($item[2]);
                } else {
                    $splitter.before.push($item[2]);
                }
            }
        });
        return $splitter;
    ...
    var check = function ($target) {
        var $splitter = splitter($target);
        var $rd = {
            prev: null,
            next: null
        };
        if (!$splitter) return $rd;
        var $index = null;
        for (var $i=0, $ilen=$splitter.target.length; $i < $ilen; $i++) {
            if (/data-selected=""/.test($splitter.target[$i])) {
                $index = $i;
                break;
            }
        }
        if ($index === null) return $rd;
        var $itrack = $index;
        while (--$itrack in $splitter.target) {
            if (!/^<!/.test($splitter.target[$itrack])) {
                $rd.prev = true;
            }
        }
        var $itrack = $index;
    ...
    var write = function ($target, $d) {
        ...
        var $cMoTxt = $splitter.target[$index].replace(/ data-selected=""/,"").replace(/\n?$/,"\n").split(/\n/);
        if ($d === -1) {
            var $itrack = $index;
            while (--$itrack in $splitter.target) {
                if (!/^<!/.test($splitter.target[$itrack])) {
                    $splitter.target[$index] = '';
                    $splitter.before = $splitter.before.concat($splitter.target.slice(0,$itrack)).join("").split(/\n/);
                    $splitter.after = $splitter.target.slice($itrack).concat($splitter.after).join("").split(/\n/);
                    $splitter.target = $cMoTxt;
                    break;
                }
            }
        } else
        if ($d === 1) {
            ...
            while (++$itrack in $splitter.target) {
                if (!/^<!/.test($splitter.target[$itrack])) {
                    $splitter.target[$index] = '';
                    $splitter.before = $splitter.before.concat($splitter.target.slice(0,$itrack+1)).join("").split(/\n/);
                    $splitter.after = $splitter.target.slice($itrack+1).concat($splitter.after).join("").split(/\n/);
                    $splitter.target = $cMoTxt;
                    break;
                }
            }
        ...
        var $tmp;
        if ($tmp = $splitter.before.pop()) $splitter.before.push($tmp);
        if ($tmp = $splitter.target.pop()) $splitter.target.push($tmp);
        SDEE.setValue([].concat($splitter.before,$splitter.target,$splitter.after).join("\n"));
        SDEE.setSelection({
                line : $splitter.before.length,
                ch : 0
            }, {
                line : $splitter.before.length + $splitter.target.length,
                ch : 0
            }
        );
        parent.SDE.File.Manager.saveTemp(parent.SDE.File.Manager.getCurrentUrl());
        return true;
    };

    var init_plugin = function () {
        if (location.pathname in plugins) {
            plugins[location.pathname]();
        }
    };

    var plugins = {
        "/layout/basic/sidebar.html": function () {
            $('#dimmedSlider').show();
